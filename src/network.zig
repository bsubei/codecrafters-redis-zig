...
pub fn readFromSocket(allocator: std.mem.Allocator, socket: posix.socket_t, buffer: *std.ArrayList(u8)) !u64 {
    // Read one chunk and append it to the raw_message.
    var num_read_bytes = try readChunk(allocator, socket, buffer);
    ...
    if (num_read_bytes == CLIENT_READER_CHUNK_SIZE) {
        num_read_bytes += try readFromSocket(allocator, socket, buffer);
    }

    return num_read_bytes;
}

pub fn readChunk(allocator: std.mem.Allocator, socket: posix.socket_t, message_ptr: *std.ArrayList(u8)) !usize {
    // Read one chunk.
    ...
    const num_read_bytes = posix.read(socket, &buf) catch |err| {
        ...
        if (err == error.WouldBlock) return readChunk(allocator, socket, message_ptr);
        return err;
    };
    try stdout.print("Read {d} bytes: {s}\n", .{ num_read_bytes, buf[0..num_read_bytes] });

    // Save this chunk to the message.
    try message_ptr.appendSlice(allocator, buf[0..num_read_bytes]);
