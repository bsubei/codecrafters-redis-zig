...
pub const ValueTimestampPair = struct { value: V, expiry_timestamp_ms: ExpiryTimestampMs };

map: std.StringHashMap(ValueTimestampPair),

pub fn init(allocator: Allocator) Self {
    return .{ .map = std.StringHashMap(ValueTimestampPair).init(allocator) };
}
...
// TODO finish tests
test "Cache basic access patterns" {
